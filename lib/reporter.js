...
const teamsClient_1 = require("./teamsClient");
class SlackTeamsReporter {
    constructor(options = {}) {
        this.testResults = [];
    ...
    async onEnd(result) {
        ...
        if (this.slackWebhookUrl) {
            ...
            const slackMessage = await (0, aiClient_1.generateMessage)({
                jsonReport: slackReportPrompt,
                model: this.model,
                apiKey: this.apiKey,
                type: this.type,
            });
            await (0, slackClient_1.sendSlackMessage)(slackMessage);
        }
        if (this.teamsWebhookUrl) {
            const teamsReportPrompt = this.generateReportPrompt(summary, "Teams");
            const teamsMessage = await (0, aiClient_1.generateMessage)({
                jsonReport: teamsReportPrompt,
                model: this.model,
                apiKey: this.apiKey,
                type: this.type,
            });
            await (0, teamsClient_1.sendTeamsMessage)(teamsMessage);
        }
    }
    ...
    generateReportPrompt(summary, platform) {
...
exports.default = SlackTeamsReporter;
